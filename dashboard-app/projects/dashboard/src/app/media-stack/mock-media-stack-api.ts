  ...
  return {
    ...
    logs: [
        ...
        history: [
          {
            timestamp: ago(120),
            status: 'ok',
            detail: '0 file(s) can be freed (~0 GiB), 2 file(s) kept',
            applied: 0,
            exitCode: 0,
          },
        ],
        ...
        history: [
          {
            timestamp: ago(48),
            status: 'ok',
            detail: 'Checked 3, no repairs needed',
            exitCode: 0,
          },
        ],
      },
      {
        ...
        format: 'ndjson',
        schedule: 'every 15m',
        description: 'Repair stuck completed downloads',
        exists: true,
        size: 4200,
        mtime: ago(35),
        current: {
        ...
        history: [
          {
            timestamp: ago(60),
            status: 'ok',
            detail: 'Summary: 1 file(s) can be freed (~0.88 GiB), 1 file(s) kept',
            applied: 0,
            exitCode: 0,
          },
          {
  ...
  return {
    ...
    logs: (source.logs ?? []).map(
      (entry): MediaStackCronLogEntryDto => ({
        ...
        current: {
          ...entry.current,
          highlights: entry.current.highlights ? [...entry.current.highlights] : undefined,
        },
...
export class MockMediaStackApi implements MediaStackApi {
  ...
  listAiPicks(_signal?: AbortSignal): Promise<AiPicksDiscover> {
    ...
    return this.withLatency(
      mapAiPicksDiscover({
        ok: true,
        items: this.aiPickItems.map((item) => ({ ...item })),
  ...
  requestMedia(payload: DiscoverRequestPayload): Promise<DiscoverAction> {
    ...
    if (aiPickItem?.request_state === 'requested' || this.requestedKeys.has(identityKey)) {
      return Promise.resolve(
        mapDiscoverAction({
          ok: true,
          message: 'Already requested',
          jellyseerr_request_id: aiPickItem?.jellyseerr_request_id ?? null,
