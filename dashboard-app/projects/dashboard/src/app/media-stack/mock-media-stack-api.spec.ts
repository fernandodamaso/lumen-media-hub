...
describe('MockMediaStackApi', () => {
  ...
  it('isolates discover fixtures across Hermes and external filters', async () => {
    ...
    expect(hermes.items.find((item) => item.id === 'hermes-eligible')).toMatchObject({
      media_status: 'missing',
      service: null,
    });
    expect(hermes.items.find((item) => item.id === 'hermes-in-library')).toMatchObject({
      media_status: 'available',
      service: 'jellyfin',
    });
    const requested = hermes.items.find((item) => item.id === 'hermes-requested');
