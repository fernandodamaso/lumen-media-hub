...
- `base_revision` must equal the store's current `revision` from the authenticated internal `GET http://localhost:8085/internal/discover/hermes` generation snapshot; a stale value returns **409** `{"ok": false, "error": "stale_base_revision", "current_revision": N}` and commits nothing. Re-read, rebuild, resubmit. The browser uses `/api/discover/hermes` and must not be used as this snapshot. Browser `/api/internal/*` requests return **404**; direct internal requests without the token return **401**.
...
- `"retain": true` keeps an **already-active eligible** item active and may refresh its `type`/`title`/`year`/`reason`; feedback, request state, all timestamps, and Jellyseerr identifiers are preserved untouched.
- Active items omitted from the batch are **rotated**: `active=false`, everything else preserved, nothing deleted — **except unrelated untouched actives** (`feedback` and `request_state` both null) that are outside the tracked, library, and watched typed deny sets. Those eligible untouched actives remain protected and retained. An active identity in any deny set is not protected, even when liked: an explicit `"retain": true` for it is rejected, and a successful generation rotates it to History while preserving its feedback, request state, timestamps, and other metadata. Rotated identities stay in `presented_media_ids`, so they can never be presented as new again. A batch where every candidate is rejected still commits and may rotate other interacted actives (`disliked` / `skipped` / `watched` / requested); Hermes recovers on its next run by submitting a valid batch.
- New candidates whose identity is already in Sonarr/Radarr are rejected as `already_tracked`; identities already playable in Jellyfin are rejected as `already_in_library`; Trakt-watched identities are rejected as `already_watched`. The authenticated internal `GET http://localhost:8085/internal/discover/hermes` generation snapshot supplies the typed `tracked_media_ids`, `in_library_media_ids`, and `watched_media_ids` deny sets, complete `required_retain` candidate objects, revision, and other generation-only context; do not expose these identity sets to the browser.
...
- **Jellyseerr is the only current request writer.** `POST /discover/request` accepts typed TMDB identity plus TV season choices, persists `request_provider=jellyseerr` in schema v4 when a Hermes id is present, and reconciles partial persistence without falling back to direct Arr writes. `arr_legacy` remains valid only for migrated historical rows.
- Browser Discover responses are decorated with authoritative `media_status`, `service`, `service_href`, `request_id`, and `monitored` lifecycle fields. The dashboard refetches the active feed after the shared request dialog succeeds; it does not keep a component-local requested-identity authority.
