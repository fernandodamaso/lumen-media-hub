...
- New candidates whose identity is already in Sonarr/Radarr are rejected as `already_tracked`; identities already playable in Jellyfin are rejected as `already_in_library`; Trakt-watched identities are rejected as `already_watched`. `GET /discover/hermes` exposes the typed `tracked_media_ids`, `in_library_media_ids`, and `watched_media_ids` deny sets under `context`, plus `required_retain` with only eligible untouched actives, so Hermes can filter before submit.
...
- **Hermes tab** (default): shows only `source: "hermes"` items from `GET /discover/hermes`. The response also carries `revision`, `presented_media_ids`, optional `generation_request`, and a `context` object (`tracked_media_ids`, `in_library_media_ids`, `watched_media_ids`, `required_retain`, `taste`) for generation clients.
...
Daily schedule: `0 10 * * *` — job prompt is a **pointer** to that file (do not inline the markdown). Hermes reads `GET /discover/hermes` for the snapshot, revision, and `context` (typed Arr/Jellyfin/Trakt-watched deny sets + `required_retain` + taste), filters Trakt picks against `presented_media_ids` and those sets, and uses the authenticated actions-API Trakt proxy's normalized `items[].type` and `items[].tmdb_id` fields rather than raw Trakt IDs. It submits candidates to `POST /discover/hermes/generations` (retrying a 409 only after a fresh snapshot), treats rejections as authoritative, logs to `tmp/hermes-discover.log`, then `POST /discover/hermes/sync`. It never writes `recommendations.json` directly — the prompt is a quality and workflow specification, not a data-integrity boundary; the API enforces the invariants.
